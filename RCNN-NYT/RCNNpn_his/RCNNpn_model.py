...
class Model(object):
    ...
    def inference(self, words_seq,left_context, right_context, features_before, batch_size, keep_prob):
        """
        :param 
        ...
        features = tf.reshape(features_pooled, (batch_size, self._feature_size))

        if features_before is not None:
            if tf.__version__[0] == '0':
                concat_feature = tf.concat(1, [features_before, features])
            else:
                concat_feature = tf.concat([features_before, features], 1)
            weights = myTF.variable_with_weight_decay('weights',
                                                      tf.truncated_normal(
                                                          [concat_feature.get_shape()[1].value, self._feature_size],
                                                          stddev=0.1),
                                                      wd=None, dtype=tf.float32)
            biases = myTF.variable_with_weight_decay('biases',
                                                     tf.constant(0.1, shape=[self._feature_size]),
                                                     wd=None, dtype=tf.float32)
        ...
        with tf.name_scope("output"):
            weights = myTF.variable_with_weight_decay('weights',
                                                      tf.truncated_normal([features_dropout.get_shape()[1].value, self._num_classes], stddev=0.1),
                                                      wd=None, dtype = tf.float32)
            biases = myTF.variable_with_weight_decay('biases',
