...
def evaluate(evalDataSet,ckpt_value,eval_value,time_list):
    ...
    with tf.Graph().as_default() as g, tf.device('/gpu:0'):
        ...
        feature_size = OPTION.FEATURE_SIZE * min(time_list[-1] - 0, OPTION.DP_DEPTH)
        if feature_size > 0:
        ...
        # inference model.
        logits, _ = textcnn.inference(input_x,input_left,input_right, features_before, batch_size, keep_prob)
            ...
            with tf.Session(config=config) as sess:
                ...
                if global_step_for_restore > last_eval_ckpt:
                    ...
                    for step in range(max_steps_per_epoch):
                        test_data, test_left, test_right, test_label, test_features = evalDataSet.next_batch(OPTION.EVAL_BATCH_SIZE)
                        if feature_size > 0:
                            feed_dict = {input_x: test_data, input_y: test_label, batch_size: np.size(test_data, 0),
                                         input_left: test_left, input_right: test_right, keep_prob: 1.0,
...
def main(argv=None):
    ...
    evalDataSet = EC_INPUT.generate_eval_data(time_list[-1])
