...
def train(newTrain, checkpoint, trainDataSet,time_list):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        # Calculate loss.
        loss = EC_MODEL.calculate_loss(logits,input_y)
        ...
        for step in range(first_step, max_steps):
            train_data, train_left,train_right, train_label, train_features = trainDataSet.next_batch(OPTION.BATCH_SIZE)
            if feature_size > 0:
                feed_dict = {input_x: train_data, input_y: train_label,
                             batch_size:np.size(train_data,0), keep_prob: OPTION.DROPOUT_KEEP_PROB,
...
def main(argv=None):
    ...
    # update paras
    trainDataSet = EC_INPUT.generate_train_data(time_list[-1])
