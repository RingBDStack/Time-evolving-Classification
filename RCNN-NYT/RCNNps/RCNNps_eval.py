            ...
            with tf.Session(config=config) as sess:
                ...
                if global_step_for_restore > last_eval_ckpt:
                    ...
                    for step in range(max_steps_per_epoch):
                        test_data, test_left, test_right, test_label = evalDataSet.next_batch(OPTION.EVAL_BATCH_SIZE)
                        feed_dict = {input_x: test_data, input_y: test_label, batch_size: np.size(test_data, 0),
...
def main(argv=None):
    ...
    evalDataSet = EC_INPUT.generate_data_set(time_list, data_name = OPTION.TEST_DATA_NAME, shuffled=False)
