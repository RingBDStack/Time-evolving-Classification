...
def evaluate(evalDataSet,time,model_time,name='train'):
    ...
    with tf.Graph().as_default() as g, tf.device('/gpu:0'):
        ...
        # inference model.
        _, features = textcnn.inference(input_x,input_left,input_right, batch_size, keep_prob)
        ...
        with tf.Session(config=config) as sess:
            ...
            for step in range(max_steps_per_epoch):
                test_data, test_left, test_right = evalDataSet.next_batch(OPTION.EVAL_BATCH_SIZE)
                feed_dict = {input_x: test_data, batch_size: np.size(test_data, 0),
...
def main(argv=None):
    ...
    for one in time_list:
        evalDataSet = EC_INPUT.generate_data_set([one],shuffled=False,label_used=False, data_name = OPTION.TRAIN_DATA_NAME)
        evaluate(evalDataSet,one,model_time,name='train')
        evalDataSet = EC_INPUT.generate_data_set([one], shuffled=False, label_used=False, data_name = OPTION.TEST_DATA_NAME)
