...
def evaluate(evalDataSet,time,model_time,name='train'):
    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        ...
        # inference model.
        _, features = textcnn.inference(input_x, features_before, OPTION.EVAL_BATCH_SIZE, eval_data=True)
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            for step in range(max_steps_per_epoch):
                test_data, test_features= evalDataSet.next_batch(OPTION.EVAL_BATCH_SIZE)
...
def main(argv=None):
    ...
    for one in time_list:
        evalDataSet = TextCNNpn_input.generate_feature_data([one],model_time,shuffled=False,label_used=False,
                                                            isTrain=True)
