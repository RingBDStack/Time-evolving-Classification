...
def train(newTrain, checkpoint, trainDataSet,time_list):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        features_before = tf.placeholder(tf.float32, [None, time_list[-1], feature_size], name="features_before")
        ...
        # inference model.
        logits, _ = textcnn.inference(input_x, features_before, OPTION.BATCH_SIZE)

        # Calculate loss.
        loss = TextCNNpn_model.calculate_loss(logits,input_y)
        ...
        for step in range(first_step, max_steps):
            train_data, train_label, train_features = trainDataSet.next_batch(OPTION.BATCH_SIZE)
            ...
            _, loss_value, accuracy_value, current_global_step = sess.run([train_op, loss, accuracy,global_step],
                                     feed_dict={input_x: train_data, input_y: train_label,
                                                features_before:train_features})
...
def main(argv=None):
    ...
    # update paras
    trainDataSet = TextCNNpn_input.generate_train_data(time_list)
