...
class DataSet(object):
    ...
    def next_batch(self, batch_size, padding = True):
        ...
        if end > self._num_examples:
            if padding:
                batch_data = np.concatenate((self._data[start:self._num_examples],
                                            self._data[0:end-self._num_examples]))
                batch_features_array = np.concatenate((self._features_array[start:self._num_examples],
                                                      self._features_array[0:end-self._num_examples]))
                if self._label_used:
                    batch_labels = np.concatenate((self._labels[start:self._num_examples],
                                                  self._labels[0:end-self._num_examples]))
                ...
                self._epochs_completed += 1
                self._shuffle_dataset()
                batch_data = self._data[0:batch_size]
                batch_features_array = self._features_array[0:batch_size]
                if self._label_used:
                    batch_labels = self._labels[0:batch_size]
                self._index_in_epoch = batch_size
        elif end == self._num_examples:
