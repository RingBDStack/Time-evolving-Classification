...
import TextCNNpn_model
import TextCNNpn_input
...
def evaluate(evalDataSet,ckpt_value,eval_value,time_list):
    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        ...
        features_before = tf.placeholder(tf.float32, [None, time_list[-1], None], name="features_before")

        textcnn = TextCNNpn_model.Model(sequence_length = OPTION.SEQUENCE_LEN,
        ...
        # inference model.
        logits,_ = textcnn.inference(input_x,features_before, OPTION.EVAL_BATCH_SIZE, eval_data=True)
            ...
            with tf.Session(config=tf.ConfigProto(
                ...
                if global_step_for_restore > last_eval_ckpt:
                    ...
                    for step in range(max_steps_per_epoch):
                        test_data, test_label, test_features = evalDataSet.next_batch(OPTION.EVAL_BATCH_SIZE)
                    ...
                    total_true_value = total_true_value[0:evalDataSet.get_dataset_size()]
...
def main(argv=None):
    ...
    evalDataSet = TextCNNpn_input.generate_eval_data(time_list)
