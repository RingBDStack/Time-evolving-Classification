...
#         total_loss = TextCNN_model.calculate_loss(total_logits, total_labels)
...
def evaluate(evalDataSet,ckpt_value,eval_value):
    ...
    with tf.Graph().as_default() as g, tf.device('/gpu:0'):
        ...
        textcnn = TextCNN_model.Model(sequence_length = OPTION.SEQUENCE_LEN,
...
def main(argv=None):
    ...
    evalDataSet = TextCNN_input.generate_data_set(time_list, data_name = OPTION.TEST_DATA_NAME, shuffled=False)
