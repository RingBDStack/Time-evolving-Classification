...
def evaluate(evalDataSet,time,model_time,name='train'):
    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        ...
        # inference model.
        logits, features = textcnn.inference(input_x, eval_data=True)
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            for step in range(max_steps_per_epoch):
                ...
                predicted_value = sess.run(logits,
            ...
            detail_filename = os.path.join(OPTION.MODELPARA_DIR, 'logits_%s_%d_%d' % (name,time,model_time))
...
def main(argv=None):
    ...
    ans = input('please choose times to get logits:')
    time_list = np.array((ans.strip()).split(' '), dtype=int)
    print('...')
    for one in time_list:
        evalDataSet = TextCNN_input.generate_train_data([one],shuffled=False,label_used=False)
        evaluate(evalDataSet,one,model_time,name='train')
        evalDataSet = TextCNN_input.generate_eval_data([one], shuffled=False, label_used=False)
