...
def main(argv=None):
    ...
    for one in time_list:
        evalDataSet = TextCNN_input.generate_data_set([one],shuffled=False,label_used=False, data_name = OPTION.TRAIN_DATA_NAME)
        evaluate(evalDataSet,one,model_time,name='train')
        evalDataSet = TextCNN_input.generate_data_set([one], shuffled=False, label_used=False, data_name = OPTION.TEST_DATA_NAME)
