...
def calculate_feature_dependence_loss(prev_features, cur_features, t):
    ...
    for i in range(t):
        ...
        feature_i = tf.squeeze(tf.slice(prev_features, [0, i, 0], [-1, 1, -1]), axis=1)
        distance = calculate_tensors_distance(feature_i, cur_features, 'euclidean')
        dependencies = tf.multiply(distance, smoothness_decay, name='smoothness_loss_para')
