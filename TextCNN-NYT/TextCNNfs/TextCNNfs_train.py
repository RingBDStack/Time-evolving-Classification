...
def train(newTrain, checkpoint, trainDataSet,time_list):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        # inference model.
        logits, cur_features = textcnn.inference(input_x)

        # Calculate loss.
        loss = TextCNNfs_model.calculate_loss(logits, input_y, features_before, cur_features, time_list[-1])
...
def main(argv=None):
    ...
    # update paras
    trainDataSet = TextCNNfs_input.generate_train_data(time_list)
