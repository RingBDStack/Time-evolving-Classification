...
import TextCNNps_model
import TextCNNps_input
...
def evaluation_result(data_path,total_predicted_value,total_true_value, total_loss, global_step, best_eval_value,summary,
                      paramaters_list, time_list):
    ...
    fr = open(os.path.join(OPTION.MODELPARA_DIR, 'para_%d' % time_list[-1]), 'w')
    for one in paramaters_list:
        for j in one:
            print(j, end=' ', file=fr)
        print(file=fr)
    fr.close()
    ...
    if accuracy > best_eval_value:
        print("enter " + datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
        ...
        if os.path.exists(targetFile):
            os.remove(targetFile)
        print("to copy->model.ckpt-best.meta" + datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
        shutil.copy(sourceFile, targetFile)
        sourceFile = os.path.join(OPTION.MODELPARA_DIR, 'para_%d' % time_list[-1])
        targetFile = os.path.join(OPTION.MODELPARA_DIR, 'para_%d_best' % time_list[-1])
        if os.path.exists(targetFile):
            os.remove(targetFile)
        shutil.copy(sourceFile, targetFile)
...
def evaluate(evalDataSet,ckpt_value,eval_value,time_list):
    ...
    with tf.Graph().as_default() as g, tf.device('/gpu:0'):
        ...
        textcnn = TextCNNps_model.Model(sequence_length = OPTION.SEQUENCE_LEN,
        ...
        # inference model.
        logits = textcnn.inference(input_x,eval_data=True)
                ...
                if global_step_for_restore > last_eval_ckpt:
                    ...
                    for step in range(max_steps_per_epoch):
                        ...
                        predicted_value, true_value, loss_value = sess.run([logits, input_y, loss],
                                             feed_dict={input_x: test_data, input_y: test_label})
                    ...
                    paramaters_list = sess.run(paramaters_list_reshape)
...
def main(argv=None):
    ...
    evalDataSet = TextCNNps_input.generate_data_set(time_list, data_name = OPTION.TEST_DATA_NAME, shuffled=False)

    print('evaluating...')

    evaluate(evalDataSet,ckpt_value,eval_value,time_list)
    open("EVAL_SUCCEED", "w")
