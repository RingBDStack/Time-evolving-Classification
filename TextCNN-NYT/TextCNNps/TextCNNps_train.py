...
def train(newTrain, checkpoint, trainDataSet,time_list):
    ...
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        # inference model.
        logits = textcnn.inference(input_x)

        # Calculate loss.
        loss = TextCNNps_model.calculate_loss(textcnn, time_list[-1], logits,input_y)
...
def main(argv=None):
    ...
    # update paras
    trainDataSet = TextCNNps_input.generate_data_set(time_list, data_name = OPTION.TRAIN_DATA_NAME, shuffled=True)
